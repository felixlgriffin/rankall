...
if (outcome == "heart attack" || outcome == "heart failure" || outcome == "pneumonia") {
 ...
 ## Begin the ranking by outcome
    if (outcome == "heart attack") {
        ...
        for (i in 1: length(unique_States)) {
#             if (num == "worst") {
#                 subState[[i]] <- subState[[i]][order(subState[[i]]$Hospital.30.Day.Death..Mortality..Rates.from.Heart.Attack,subState[[i]]$Hospital.Name, subState[[i]]$State,decreasing=TRUE)]
#         num <- 1
#             }
        final[i,] <- subState[[i]][num[i],1:2]
    }
    return(final)
    }
